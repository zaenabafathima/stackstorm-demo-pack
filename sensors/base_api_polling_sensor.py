...
class ApiPollingSensorBase(PollingSensor):
    ...
    def make_request_with_retry(
        ...
        try:
            ...
            err_msg = "Request Failed with status code {status_code},"\
            ...
            try:
                ...
                print("cannot complete request due to {0}".format(str(exc)))
            ...
            while retry_count > 0 and (
                ...
                print(
                    "sleeping for {0} seconds, retry count={1}".format(
                        cur_retry_wait_time, retry_count
                    )
                )
                time.sleep(cur_retry_wait_time)
                LOGGER.info(
                    "making request now, remaining retry counts {0}".format(
                        retry_count
                    )
                )
                start_time = time.time()
            ...
            if response.status_code in [200, 201]:
                LOGGER.info(
                    "Total time taken for api is {0} seconds".format(
            ...
            if (
                type(exc).__name__
                != requests.exceptions.ConnectionError.__name__
            ):

                LOGGER.exception(
                    "Unhandled exception in make_request_with_retry"
                )
            raise
